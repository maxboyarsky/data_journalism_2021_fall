...
**A2.** California, Texas, Florida, NY, PA, Illinois, Georgia, North Carolina. Michigan, New Jersey.
...
state_ppp_all %>%
  ggplot() +
  geom_bar(aes(x=reorder(state,total_ppp_loans), weight=total_ppp_loans)) +
  coord_flip() + 
  labs(
    title="States with the Most PPP Loans",
    x = "state",
    y = "number of loans",
    caption = "source: SBA PPP loan database"
...
**A3.**  
```{r}
loans_under_25<-lab_09 %>%
  filter(amount<25000)

loans_under_25 %>%
  ggplot(iris, aes_(x=amount, y=current_approval_amount)) + 
    geom_point()
