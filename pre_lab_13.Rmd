...
**Answer** Shows the state, the percentage of voters who voted for either candidate in each state, the percentage of businesses who received PPP loans, and the ppe amount per employee. 
```{r}
...
**Answer** General trend that shows some correlation between trump's voting percentage per state and businesses receiving PPP loans.  
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Less of a correlation between the two. The line of best fit is flattened. 
```{r}
...
**Answer** A somehwat strong positive correlation and a somewhat weak positive correlation. 
```{r}
reveal_data %>%
...
**Answer** More of a correlation, but there's less of a chance that this is the statistical norm, as the p value is low. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Less of a correlation, but more of a chance that this is a statistical norm, as the p value is greater. 
```{r}
...
**Answer** 
The average percentage of businesses in either Trump or Clinton states that received PPP loans compared to the average amount of ppp loans per employee in those states. In Clinton states, the number is smaller for both. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The p value is high which indicates the the correlation is reliable. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The p and t value is quote low which indicates the correlation is not meaningful or reliable. 
```{r}
