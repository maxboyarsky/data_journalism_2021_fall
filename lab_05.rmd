...
```{r}
West_Virginia_Loan <- read_rds("data/lab_05.rds")
Naics_Code <- read_csv("data/naics_codes.csv")
WV_Population <- read_csv("data/wv_population_county.csv")
Data_Zip_Loan_Sample <- read_csv("data/zip_25401_loan_sample.csv")
```
...
```{r}
Cleaned_Address<-read_csv("data/Cleaned Address.csv")
```


Then answer these questions:
* What is the street address in this data that has the most loans? 126 E Burke Street
* How many loans are there at that street address? 6
* What are the names of the businesses at that address?
DREW HOLDINGS, BRIX27, ABRAHAM HOUSTON, HUB CO-OP, RONIN PROPERTIES

**A1.**
```{r}
Cleaned_Address %>% 
  group_by(name) %>%
 filter(str_detect(address,"^126 E"))
...
**A2.** Yes, especially considering that most of the companies are LLC's and Sole Proprietor ships and should not share the same complex space. I looked up some of the businesses' websites as well, they all don't seem very legitimate.One is also a person, Abraham Ashton.   
